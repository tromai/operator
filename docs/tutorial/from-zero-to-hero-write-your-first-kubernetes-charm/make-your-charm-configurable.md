...
We also add error handling for getting the workload version. If we cannot access the workload to get its version, a `RuntimeError` will be raised. We handle this by logging the error, and set the unit status to `blocked` with a message prompting the users to check incorrect port configuration.
...
We needs the `mock_version` fixture because `_on_config_changed` calls `_replan_workload`, which gets the workload version. The fixture patches `get_version` to avoid making a real HTTP call.
